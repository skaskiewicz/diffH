...
def export_to_geopackage(
    ...
    try:
        ...
        if "h_odniesienia" in df_geo.columns:
            df_geo["h_odniesienia"] = pd.to_numeric(
                df_geo["h_odniesienia"], errors="coerce"
            ).round(round_decimals)
        if "diff_h_geoportal" in df_geo.columns:
            df_geo["diff_h_geoportal"] = pd.to_numeric(
                df_geo["diff_h_geoportal"], errors="coerce"
            ).round(round_decimals)
        if "diff_h" in df_geo.columns:
            df_geo["diff_h"] = pd.to_numeric(df_geo["diff_h"], errors="coerce").round(
                round_decimals
            )
        # Dodaj/aktualizuj kolumnę 'eksport' zgodnie z warunkiem dokładności
        if "osiaga_dokladnosc" in df_geo.columns:
            df_geo["eksport"] = df_geo["osiaga_dokladnosc"].apply(
                lambda x: True if str(x).strip().lower() == "tak" else False
            )
        else:
            df_geo["eksport"] = True
        ...
        print(
            f"{Fore.GREEN}Wyniki zostały poprawnie zapisane w bazie przestrzennej: {os.path.abspath(gpkg_path)}{Style.RESET_ALL}"
        )
        # 2. Eksport tylko spełniających warunek dokładności
        gdf_ok = gdf[gdf["eksport"]]
        if not gdf_ok.empty:
            gdf_ok.to_file(
                gpkg_path.replace(".gpkg", "_dokladne.gpkg"),
                layer=layer_name,
                driver="GPKG",
            )
            print(
                f"{Fore.GREEN}Wyniki spełniające warunek dokładności zapisano w: {os.path.abspath(gpkg_path.replace('.gpkg', '_dokladne.gpkg'))}{Style.RESET_ALL}"
            )
        else:
            print(
                f"{Fore.YELLOW}Brak punktów spełniających warunek dokładności do eksportu."
            )
        # 3. Eksport niespełniających warunku dokładności
        gdf_nok = gdf[~gdf["eksport"]]
        if not gdf_nok.empty:
            gdf_nok.to_file(
                gpkg_path.replace(".gpkg", "_niedokladne.gpkg"),
                layer=layer_name,
                driver="GPKG",
            )
            print(
                f"{Fore.GREEN}Wyniki niespełniające warunku dokładności zapisano w: {os.path.abspath(gpkg_path.replace('.gpkg', '_niedokladne.gpkg'))}{Style.RESET_ALL}"
            )
        else:
            print(
                f"{Fore.YELLOW}Brak punktów niespełniających warunku dokładności do eksportu."
            )
    except Exception as e:
